    ...
    <body><center>
		...
		<?php
			function databaseLoad(){
				...
				$resultItems = mysqli_query($conn, "SELECT id, item FROM nameTable;");
				...
				while($row = mysqli_fetch_assoc($resultItems)) {
					echo $row["id"] . ": '" . addslashes($row["item"]) . "',";
				}
				echo "}</script>";
				
				$resultRelationship = mysqli_query($conn, "SELECT * FROM relationship;");
