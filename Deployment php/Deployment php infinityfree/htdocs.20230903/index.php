    ...
    <body><center>
		...
		<?php
			function databaseLoad(){
			    ...
			    $username = "epiz_32810590";
			    $password = "gn26VANGNJ";
			    $dbname = "epiz_32810590_mosverkstad";
				$conn = mysqli_connect($servername, $username, $password, $dbname);
				mysqli_set_charset($conn, "utf8");
				if (!$conn) {
					die("Connection failed: " . mysqli_connect_error());
				}
				$resultItems = mysqli_query($conn, "SELECT id, item FROM epiz_32810590_mosverkstad.nameTable;");
				...
				$resultRelationship = mysqli_query($conn, "SELECT * FROM epiz_32810590_mosverkstad.relationship ORDER BY sortid;");
				if (mysqli_num_rows($resultRelationship) <= 0){
					echo "<script>var dbRelationship = []</script>";
					return;
				}
				$myresult = mysqli_query($conn, "SELECT E1.id AS parentid, E1.item AS parent, E2.id AS childid, E2.item AS child FROM epiz_32810590_mosverkstad.relationship AS R LEFT JOIN epiz_32810590_mosverkstad.nameTable E1 ON R.parentId = E1.id LEFT JOIN epiz_32810590_mosverkstad.nameTable E2 ON R.childId = E2.id ORDER BY R.parentId, R.sortId, R.childId;");
