    ...
    <body><center>
		...
		<?php
			function databaseLoad(){
				...
				$resultItems = mysqli_query($conn, "SELECT id, item FROM temp.nameTable;");
				...
				$resultRelationship = mysqli_query($conn, "SELECT * FROM temp.relationship ORDER BY sortid;");
				if (mysqli_num_rows($resultRelationship) <= 0){
					echo "<script>var dbRelationship = []</script>";
					return;
				}
				$myresult = mysqli_query($conn, "SELECT E1.id AS parentid, E1.item AS parent, E2.id AS childid, E2.item AS child FROM temp.relationship AS R LEFT JOIN temp.nameTable E1 ON R.parentId = E1.id LEFT JOIN temp.nameTable E2 ON R.childId = E2.id ORDER BY R.parentId, R.sortId, R.childId;");
